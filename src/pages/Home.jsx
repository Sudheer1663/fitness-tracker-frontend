...
function Home() {
  ...
  const handleGetStarted = () => {
    ...
    if (user?.getstarted && goal) {
      // Already completed get started, go directly to goal
      ...
      // First time, go to getstarted page
      navigate("/getstarted");
    }
  };
  ...
  return (
    <div className={`home-container ${darkMode ? "dark-mode" : "light-mode"}`}>
      <section className="hero">
        <h1>Welcome to Your Fitness Tracker</h1>
        <p>Track your activity, monitor your health, and stay motivated every day!</p>
      ...
      <section className="cta-section">
        <h2>Start Your Journey Today</h2>
        <p>Your health transformation starts with one step. Join us and stay fit for life!</p>
        <button onClick={() => navigate("/signup")}>Join Now</button>
      </section>
    </div>
  );
